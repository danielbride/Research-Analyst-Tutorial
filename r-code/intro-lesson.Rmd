...
You just ran a `r gdef("functional_programming", "function")` using R code! You provided the **input** "Hello, World!" to the `print()` function and saw
...
`r gdef("argument", "arguments")`. The function subtracts the second argument
from the first.
...
to load `r gdef("package", "packages")`, which are really just collections
(libraries, if you will) of functions that aren't available until you make 
them available. 

`r gdef("base-r", "Base R")` provides a number of functions that can do some 
...
The `r gdef("tidyverse", "tidyverse")` is a collection of several packages 
...
and `y` **variables**. The concept of a `r gdef("variable_program", "variable")` 
